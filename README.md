...
Drives Hermes Agent chat sessions with full TUI gateway support — streaming responses, tool-step display,
and interactive decision gates (tool-approval, confirm/select dialogs) — as a clean, modern chat interface.
...
The plugin is installed into `~/.hermes/plugins/hermes-chat/`. `hermes hermes-chat start` spawns a
...
| Command | Purpose |
|---|---|
| `hermes hermes-chat start` | Start the background daemon with health watchdog |
...
| `hermes hermes-chat install-deps` | Install missing Python dependencies into the Hermes environment |
| `hermes hermes-chat test-gates -v` | Run gate-detection dev tests against representative questionary prompts |

### Manual / development install

```bash
git clone https://github.com/roormonger/hermes-bridge.git
cd hermes-bridge  # repo name unchanged
...
The plugin also adds a **Hermes Chat** tab to the Hermes web dashboard. Open the dashboard after
enabling the plugin and look for the new tab (it may take a dashboard refresh / rescan to appear).
The tab shows:

- Live daemon status and health.
- Current bridge config.
- Start / stop / restart / install-deps buttons.
- The last 100 lines of the daemon log.
...
Hermes Chat reads its runtime settings from `~/.hermes/plugins/hermes-chat/config.yaml` (or
